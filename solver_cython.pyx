...
cpdef tuple find_piece(np.ndarray[DTYPE_t, ndim=3] image,
                       np.ndarray[DTYPE_t, ndim=3] template,
                       double scale, tuple blur_size, object logger):
...
cdef tuple find_shape_points(np.ndarray[DTYPE_t, ndim=2] image, tuple piece_loc,
                             int sx, int sy, double scale, object logger):
    cdef int[2] start_point = [0, sy]
    cdef int x, y
    ...
    cdef int[2] end_point = list(start_point)
    start_point[0] = (start_point[0] + sx) / 2

    # Find end point
    cdef int vp = 0  # Vertical pixels
    cdef int new_x
